...
class PreProc(Scene):
    def construct(self):
        ...
        dots = [
            Dot(single, color=BLUE if single[2] == 1 else YELLOW) for single in data
        ]
        ...
        descN = Text("二分类\nBinary Classification", font=DEFAULT_FONT).shift(
            DOWN + 2 * LEFT
        )
        ...
        desc = MathTex(r"\vec{x}^{(1)} = (", r"1", r",", r"1", r")").shift(
            2 * LEFT + UP
        )
        self.play(Write(desc))
        self.wait()
        ...
        framebox1 = SurroundingRectangle(desc[1], buff=0.1)
        framebox2 = SurroundingRectangle(desc[3], buff=0.1)
        descN = MathTex(r"x_1^{(1)}").shift(2 * LEFT + DOWN)
        desc2 = Text("第 1 个向量的第 1 个维度", font=DEFAULT_FONT).shift(
            2 * LEFT + 2 * DOWN
        )
        ...
        desc2 = Text("第 1 个向量的第 2 个维度", font=DEFAULT_FONT).shift(
            2 * LEFT + 2 * DOWN
        )
        self.play(Write(descN), Write(desc2))
        self.wait(0.5)
        ...
        arrow = Arrow(ORIGIN, [1, 1, 0], buff=0, color=BLUE)
        self.play(
            ReplacementTransform(arrow2, arrow),
            FadeOut(descN),
            FadeOut(desc),
            FadeOut(desc2),
        )
        ...
        framebox1 = SurroundingRectangle(desc[1], buff=0.1)
        framebox2 = SurroundingRectangle(desc[3], buff=0.1)
        framebox3 = SurroundingRectangle(desc[5], buff=0.1)
        self.play(Create(framebox1))
        self.wait(0.5)
        self.play(ReplacementTransform(framebox1, framebox2))
        self.wait(0.5)
        ...
        desc2 = MathTex(
            r"\left\{\left\{x_1^{(i)}, x_2^{(i)}, y^{(i)} \right\}\right\}, ",
            r"x_j^{(i)} \in \mathbb{R}, y^{(i)} \in \{-1, 1\}",
        ).shift(2 * DOWN)
        self.play(Write(desc2))
        self.wait()
...
class ALine(Scene):
    def construct(self):
        ...
        def get_line():
            ax = Axes(x_range=[-10, 10], y_range=[-5, 5], tips=True)
            line = ax.plot(
                lambda x: -(a1T.get_value() * x + bT.get_value()) / a2T.get_value(),
                color=BLUE,
            )
            return line
        ...
        ax2 = Axes(x_range=[-10, 10], y_range=[-5, 5], tips=True)
        self.play(Create(ax2))
        desc2 = MathTex("a_1", "x", " + ", "a_2", "y", " + ", "b", " = 0").shift(
            UP * 3 + LEFT * 3
        )
        self.play(Write(desc2))
        self.wait()
        ...
        def get_arrow():
            arr = Arrow(
                ORIGIN, ax2.c2p(a1T.get_value(), a2T.get_value(), 0), buff=0, color=BLUE
            )
            return arr

        arr = always_redraw(get_arrow)
        tip = MathTex(f"({a1.get_value()}, {a2.get_value()})").next_to(
            arr.get_end(), RIGHT
        )

        self.play(
            Create(arr),
            a1L.animate.set_color(BLUE),
            a2L.animate.set_color(BLUE),
            a1.animate.set_color(BLUE),
            a2.animate.set_color(BLUE),
        )
        self.wait(0.5)
        self.play(Write(tip))
        self.wait(0.5)
        ...
        self.wait(0.5)
        exp = MathTex(r"0", "=", r"\vec{w}", r"\cdot", r"\vec{x}", "+", "b").shift(
            LEFT * 3 + DOWN * 3
        )
        # self.play(Write(exp))
        ...
        initial_point = [
            ax2.c2p(
                t.get_value(),
                -(a1.get_value() * (t.get_value()) + bT.get_value()) / a2.get_value(),
            )
        ]
        ...
        dot.add_updater(
            lambda d: d.move_to(
                ax2.c2p(
                    t.get_value(),
                    -(a1.get_value() * (t.get_value()) + bT.get_value())
                    / a2.get_value(),
                )
            )
        )
        ...
        dot.add_updater(
            lambda d: d.move_to(
                ax2.c2p(
                    t.get_value(),
                    -(a1.get_value() * 1 + bT.get_value()) / a2.get_value(),
                )
            )
        )
        ...
        tip.add_updater(
            lambda l: l.set_value(
                a1.get_value() * t.get_value()
                + a2.get_value() * currentY
                + b.get_value()
        ...
        area1 = ax2.get_area(
            ax, [-10, 10], color=BLUE, bounded_graph=ax2.plot(lambda _: 8)
        )
        self.play(FadeIn(area1), t.animate.set_value(3))
        self.wait()
        area2 = ax2.get_area(
            ax, [-10, 10], color=YELLOW, bounded_graph=ax2.plot(lambda _: -8)
        )
        ...
        exp2 = MathTex(r"\hat{y}", r" = \vec{w} \cdot \vec{x} + b").shift(
            LEFT * 3 + DOWN * 3
        )
        self.wait(0.5)
        ...
        self.play(
            *[FadeOut(item) for item in [arrow_value, arr_op, arr_value]],
            Write(fin_arr_value),
        )
        self.wait()

        self.play(
            a1T.animate.set_value(0),
            a2T.animate.set_value(2),
            fin_arr_value.animate.move_to(ax2.c2p(1, 2, 0)),
        )
        self.wait()
        ...
        self.play(
            a1T.animate.set_value(a1T.get_value() - 2),
            a2T.animate.set_value(a2T.get_value() - 1),
        )
        ...
        self.play(
            FadeOut(arrow_tip),
            sample.animate.set_color(BLUE),
            Flash(sample, color=BLUE),
        )
        ...
        cond1 = Text(
            "如果预测与实际相同，不做动作", font=DEFAULT_FONT, font_size=30
        ).shift(4 * LEFT + 3.5 * UP)
        cond2 = Text(
            "如果预测与实际不同，调整参数", font=DEFAULT_FONT, font_size=30
        ).next_to(cond1, DOWN)
        self.play(AnimationGroup(Write(cond1), Write(cond2), lag_ratio=0.5))
        self.wait()

        method1 = MathTex(
            r"\hat{y} > 0, y < 0 \;\Rightarrow\; ",
            r"\vec{w}' = \vec{w} - \vec{x}",
            r" \\ ",
            r"b'=b - 1",
            font_size=37,
        ...
        self.play(
            Write(method1),
            sample.animate.set_color(YELLOW),
            Flash(sample, color=YELLOW),
            FadeIn(area),
        )
        self.wait()
        self.play(
        ...
        self.wait()
        self.play(bT.animate.set_value(bT.get_value() - 1), Indicate(method1[3]))
        self.wait()
        self.play(FadeOut(area))
        ...
        self.wait()
        self.play(bT.animate.set_value(bT.get_value() + 1), Indicate(method2[3]))
        self.wait()
        self.play(FadeOut(area))
...
class Perc(Scene):
    def construct(self):
        ...
        tlossLable = DecimalNumber(
            0.4, show_ellipsis=False, num_decimal_places=2, include_sign=False
        ...
        self.play(
            ReplacementTransform(exp2, exp),
            t.animate.set_value(2.5),
        ...
        mean = MathTex(r"\text{Mean:} \mu_{x_1} = 2.8 \\ \mu_{x_2} = 2.2").shift(
            3 * RIGHT + DOWN
        )
        std = MathTex(r"\text{Std:} \sigma_{x_1} = 1.2 \\ \sigma_{x_2} = 0.7").shift(
            3 * RIGHT + 3 * DOWN
        )
        ...
        exp2 = (
            MathTex(
                r"x'_{1} = \frac{x_{1} - 2.8}{1.2} \\ x'_{2} = \frac{x_{2} - 2.2}{0.7}"
            )
            .shift(3 * LEFT + 2 * UP)
            .next_to(exp, DOWN)
        )
        ...
        dataN = [
            [(single[0] - 2.8) / 1.2, (single[1] - 2.2) / 0.7, single[2]]
            for single in data
        ]
        dotsN = [
            Dot(single, color=BLUE if single[2] == 1 else YELLOW) for single in dataN
        ]
        self.play(
            *[ReplacementTransform(dots[i], dotsN[i]) for i in range(len(dots))],
            FadeOut(mean),
            FadeOut(std),
            FadeOut(exp2),
            exp.animate.move_to(3 * UP).set_color(RED),
        )
        self.wait()

        desc = Text("归一化: “Z-score 标准化”方法", font=DEFAULT_FONT, color=RED).shift(
            2 * UP
        )
        self.play(Write(desc))
        self.wait()
...
class Ntwk(Scene):
    def construct(self):
        ...
        inputText = Text("输入层", font=DEFAULT_FONT, font_size=20).shift(
            2 * LEFT + 3.5 * UP
        )
        ...
        exp = MathTex(r"f \left( \sum w_i x_i - b \right)").shift(
            3 * RIGHT + 0.7 * DOWN
        )
        self.play(
            FadeOut(bias),
...
class AcFun(Scene):
    def construct(self):
        ...
        labels = ax.get_axis_labels(
            Text("总输入", font=DEFAULT_FONT).scale(0.7),
            Text("输出", font=DEFAULT_FONT).scale(0.7),
        )
        ...
        name2 = (
            Text("Sigmoid 函数", font=DEFAULT_FONT)
            .next_to(title, DOWN)
            .shift(0.8 * RIGHT)
        )
        ...
        name3 = (
            Text("ReLU 函数", font=DEFAULT_FONT).next_to(title, DOWN).shift(0.2 * RIGHT)
        )
...
class LRate(Scene):
    def construct(self):
        ...
        label = ax.get_axis_labels(
            Text("参数", font=DEFAULT_FONT).scale(0.7),
            Text("学习效果", font=DEFAULT_FONT).scale(0.7),
        )
        ...
        lrateTip = Text("学习率:", font=DEFAULT_FONT, font_size=30).shift(
            5 * LEFT + 3 * UP
        )
        ...
        lfuncTip = Text("衰减函数:", font=DEFAULT_FONT, font_size=30).next_to(
            lrateTip, DOWN, aligned_edge=LEFT
        )
        lfuncVal = (
            MathTex(r"f(\eta) = 0.5 \times \eta").scale(0.8).next_to(lfuncTip, RIGHT)
        )
        ...
        for _ in range(5):
            self.play(
                t.animate.set_value(
                    t.get_value() + (1 if t.get_value() < 0 else -1) * lrate.get_value()
                )
            )
...
class XOR(Scene):
    def construct(self):
        ...
        dots = [
            Dot(ax.c2p(*single), color=BLUE if single[2] == 1 else YELLOW)
            for single in data
        ]
        self.play(*[Create(dot) for dot in dots])
        self.wait()
...
class MLP(Scene):
    def construct(self):
        ...
        self.play(
            *[Create(arrow) for arrow in [arr1, arr2, arr3, arr4, arr5, arr6, arr7]]
        )
        self.wait()
        ...
        biasTip = MathTex("b = 0.5").next_to(
            yellowTip, DOWN, buff=0.5, aligned_edge=RIGHT
        )
        ...
        # input
        self.play(
            x1Val.animate.move_to([-2.5, 1, -1]), x2Val.animate.move_to([-2.5, -1, 0])
        )
        self.wait()

        # light up cx1
        self.play(
            x1Val.animate.set_color(BLACK), cx1.animate.set_fill(WHITE, opacity=1)
        )
        self.wait()
        ...
        # move
        self.play(
            mid1Val.animate.move_to([-0.5, 1, 0]), midVal.animate.move_to([-0.5, 1, 0])
        )
        self.wait(0.5)
        ...
        # light up
        self.play(
            ch1.animate.set_fill(WHITE, opacity=1), h1Val.animate.set_color(BLACK)
        )
        self.wait()
        ...
        self.play(
            midVal.animate.move_to([-0.5, -1, 0]),
            mid2Val.animate.move_to([-0.5, -1, 0]),
        )
...
class DumyNd(Scene):
    def construct(self):
        ...
        expN = MathTex(r"f \left(\sum w_i x_i + (-1) \times b \right)").shift(
            3 * RIGHT + 2 * UP
        )
        ...
        expN = MathTex(r"= f \left( \sum_{i = 1}^{n + 1} w_i x_i \right)").shift(
            1.5 * RIGHT
        )
        ...
        origTrain = MathTex(
            r"\vec{w}' &= \vec{w} \pm \eta \vec{x} \\ b' &= b \pm \eta"
        ).shift(2 * LEFT + 1.5 * DOWN)
        self.play(Write(origTrain))
        self.wait()

        newTrain = MathTex(r"\vec{w}' = \vec{w} + \eta (y - \hat{y}) x_i").shift(
            2.5 * RIGHT + 1.5 * DOWN
        )
